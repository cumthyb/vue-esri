...
let printer = null;
let addlayer = null;
let measureEvent = null;
let dbmeasureEvent = null;
let measuremoveEvent = null;
let moveMeasureFlag = false;
//let mapevent = null;
...
*/
export function createMap(divId, mapOptions, callback){
 ...
 * 
 */
export function createPointGraphic(params){
 ...
 * 
 */
export function createLineGraphic(params){
 ...
 * 
 */
export function createPolygonGraphic(params){
...
//地图放大
export function zoomIn(callback){	
  	let zoom = null;
...
//地图缩小
export function zoomOut(callback){
	let zoom = null;
	if(HJRMap && HJRMap.map){
...
function addWMSLayer(opts){
  loadModules(['esri/layers/WMSLayer']).then(([ WMSLayer]) => {
    ...
    if (IsLayerToEmpty(opts.layerid)) {
	    ...
	    if(opts.index)
	    	HJRMap.map.addLayer(wmsLayer, index);
	    else
...
function addWFSLayer(opts){
  loadModules(['esri/layers/WFSLayer']).then(([WFSLayer]) => {
    ...
    if (IsLayerToEmpty(opts.layerid)) {
	    ...
	    if(opts.index)
	    	HJRMap.map.addLayer(wfsLayer, index);
	    else
...
function addDynamicLayer(opts){
  loadModules(['esri/layers/ArcGISDynamicMapServiceLayer']).then(([ ArcGISDynamicMapServiceLayer]) => {
    ...
    if (IsLayerToEmpty(opts.layerid)) {
	    ...
	    if(opts.index)
	    	HJRMap.map.addLayer(arcgisDynamic, index);
	    else
...
function addTileLayer(opts) {
  loadModules(['esri/layers/ArcGISTiledMapServiceLayer']).then( ([ArcGISTiledMapServiceLayer]) => {
    ...
    if (IsLayerToEmpty(opts.layerid)) {
	    ...
	    if(opts.index)
	    	HJRMap.map.addLayer(arcgisTiled, index);
	    else
	    ...
	    if (IsLayerToEmpty(opts.layerid)) {
	      ...
	      featureLayer.clear();
	    	...
	    	if (opts.url !== null || opts.url !== undefined || opts.url !== '') {
	      	...
	      	if(opts.index)
		    	HJRMap.map.addLayer(featureLayer, index);
		    else
...
*     -layerid:图层的标识
*/
function addGraphicsLayer(opts){
	loadModules(["esri/layers/GraphicsLayer"]).then(([GraphicsLayer])=>{
	    ...
	    if (IsLayerToEmpty(opts.layerid)) {
	    	...
	    	if(opts.index)
		    	HJRMap.map.addLayer(graphicLayer, opts.index);
		    else
...
*/
export  function reorderLayer(layerid, index, callback){
	try{
...
*/
export function addGraphic(params, callback){
...
*   id： 查找的graphic的id属性值
*/
export function isFindGraphicByAttr(layerId, id){
	var flag = false;
	try{
...
*  url: 鼠标样式的图片地址
*/
export function setMousePointer(url){ 		
	try{
...
*  layerid: 图层的唯一标识
*/
export function EnterviewLayer(layerid){
	try{
...
*/
export function RectQuery(params){
...
*	callback： 缩放完毕后的回调函数
*/
export function  SetZoom(zoom,callback){
...
*		-callback: 批量增加完的回调函数
*/
export function  AddGraphics(params){	
...
*		-callback: 批量修改完的回调函数
*/
export function  EditGraphics(params){
  	let flag = false;
  	var olayer = FindLayer(params.layerid);
  	if(!olayer) return;
  	let count = 0;
...
*		-callback: 批量删除完的回调函数
*/
export function RemoveGraphics(params){
	let flag = false;
	try{		
	  	var olayer = FindLayer(params.layerid);
	  	if(!olayer) return;
...
*/
export function Twinkle(params){
...
*/
export function InitTraceManager(params){
...
*/
export function TracePlay(speed){
...
*	time：播放速度，是每秒钟前进的时间
*/
export function TraceReplay(speed){
...
//生成聚合图层
export function  InitClusteringLayer(params){
	let olayer = null;
	let cdata = [];
...
*	extent: 导出地图图片的范围
*/
export function ExportMap(extent){
...
*/
export function LocationCenterAt(x, y, zoom, callback){
 ...
 */
export function ShowLabels(params, labelLayer){
...
export function ShowLabelsforFeatureLayer(params){
 ...
 */
export function  MouseClick(params){
 ...
 * 		
 */
export function InitNavSlider(container, options){
	let navnode = null;
 ...
 */
export function InitScalebar(options){
...
//显示实时位置组价
export	function InitCoordinate(domId){
...
//鹰眼
export function InitOverview(domId){
 	try{
...
//清除测量距离
export function clearMeasureDistance(){
...
//测量距离
export function StartMeasureDistance(options){
  	...
  	loadModules([
		...
		let totalLength = 0;
		let measureLayer = FindLayer('measureLayer');
		var index = HJRMap.map.getLayersVisibleAtScale().length-1;
 		reorderLayer("measureLayer", index);
		HJRMap.map.setMapCursor('crosshair');
		if(measureEvent){
			measureEvent.remove();
			measureEvent = null;
		}
		measureEvent = HJRMap.map.on('mouse-down',function(evt){
			...
			if(measuremoveEvent){
		 	...
		 	if(count == 0){
		 		...
		 		var linelength = mathUtils.getLength(lastPnt, evt.mapPoint);
				if(linelength<100)
					return;
		...
		dbmeasureEvent = HJRMap.map.on('dbl-click',function(evt){
		 	...
		 	if(tmplineGraphic)
		 		measureLayer.remove(tmplineGraphic);
		 	measureLayer.remove(measureLayer.graphics[measureLayer.graphics.length-1]);
		 	measureLayer.remove(measureLayer.graphics[measureLayer.graphics.length-1]);
...
//测量面积
export  function StartMeasureArea(options){
	let unit = options.unit;
  	loadModules([
  		"esri/toolbars/draw",
  		'esri/graphic',
		...
		"esri/geometry/geometryEngine"
 		...
 		reorderLayer("measureLayer", index);
	  	...
	  	var toolbarEvent = toolbar.on("draw-end", function(evt){
		  	...
		  	toolbar.deactivate();
		  	options.callback&&options.callback({status:"success",data:graphic});
		  	toolbarEvent.remove();
		  	toolbarEvent = null;
		  	HJRMap.map.setMapCursor('default');
		  });
...
//图层管理
export	function InitLayerList(domId){  		
	try{
