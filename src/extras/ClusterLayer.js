...
define([
  ...
  return declare([GraphicsLayer], {
    constructor: function(options) {
      ...
      this._clusterLabelOffset = (options.hasOwnProperty("labelOffset")) ? options.labelOffset : 8;
      // graphics that represent a single point
      this._singles = []; // populated when a graphic is clicked
      this._showSingles = options.hasOwnProperty("showSingles") ? options.showSingles : true;
      // symbol for single graphics
      var SMS = SimpleMarkerSymbol;
      this._singleSym = options.singleSymbol || new SMS({
				"type": "esriSMS",
				 "style": "esriSMSCircle",
				 "color": [26,215,20,155],
				 "size": 6,
			...
			singleTemp = { "title": "", "description": "{*}" };
			if(options.singleTemplate){
				singleTemp = new PopupTemplate(options.singleTemplate);
			}else{
				singleTemp = new PopupTemplate(singleTemp);
			}
      this._singleTemplate = singleTemp;
